...
public class VectorReactCGI implements ReactCGI {
  ...
  public String generat(FigmaVectorNode fNode) {
    ...
    //                colorHashGenerator.generateColorHash((int) fillsSolid.getColor().getR(), (int)
    // fillsSolid.getColor().getG(), (int) fillsSolid.getColor().getB(), (int)
    // fillsSolid.getOpacity());
    final String begin =
        "<svg xmlns=\""
            + "url"
            + "\" xmlns:xlink=\""
            ...
            + "viewBox=\"0 0 256 228\"><path fill=\""
            + fillColor
            + "\" d=\""
            + fNode.getVectorPaths().get(0).getData()
            + "";
    return genCode;
  }
}
